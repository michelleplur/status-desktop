...
method suggestedRoutes*(self: AccessInterface, account: string, amount: float64, token: string, disabledChainIDs: seq[uint64], preferredChainIDs: seq[uint64]): string {.base.} =
  raise newException(ValueError, "No implementation available")
